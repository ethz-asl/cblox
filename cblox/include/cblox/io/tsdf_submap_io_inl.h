...
bool LoadSubmapFromStream(
  ...
  LOG(ERROR) << "Could not read protobuf message of this submap type.";
  return false;
}

template<>
bool LoadSubmapFromStream<TsdfSubmap>(
    std::fstream* proto_file_ptr,
...
template<>
bool LoadSubmapFromStream<TsdfEsdfSubmap>(
    std::fstream* proto_file_ptr,
    typename SubmapCollection<TsdfEsdfSubmap>::Ptr submap_collection_ptr,
    uint32_t* tmp_byte_offset_ptr) {
  CHECK_NOTNULL(proto_file_ptr);
  CHECK(submap_collection_ptr);
  CHECK_NOTNULL(tmp_byte_offset_ptr);
